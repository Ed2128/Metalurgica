...
export default function Presupuestos() {
  // Pestaña activa: 'nuevo' o 'historial'
  const [pestana, setPestana] = useState<'nuevo' | 'historial'>('nuevo');

  // Datos maestros de la BD
  ...
  // Estado para el modal de vista previa e impresión
  ...
  const guardarPresupuesto = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (respuesta.ok) {
        ...
        setClienteId('');
        setItems([]);
        cargarHistorial(); // Refrescamos el historial
        setPestana('historial'); // Cambiamos de pestaña para verlo
      } else {
  ...
  const abrirImpresion = (orden: any) => {
    setPresupuestoSeleccionado(orden);
  };
  ...
  return (
    <div className="space-y-6 print:p-0">
      ...
      {/* CABECERA Y PESTAÑAS (Ocultas al imprimir) */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 print:hidden">
        ...
        <div className="flex bg-gray-200 p-1 rounded-lg">
          <button 
            ...
            className={`flex items-center gap-1.5 px-4 py-2 rounded-md font-medium text-sm transition-colors ${pestana === 'nuevo' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-600 hover:text-gray-800'}`}
          >
            <Plus size={16} /> Nuevo Presupuesto
          </button>
          <button 
            onClick={() => setPestana('historial')}
            className={`flex items-center gap-1.5 px-4 py-2 rounded-md font-medium text-sm transition-colors ${pestana === 'historial' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-600 hover:text-gray-800'}`}
          >
          ...
          <form onSubmit={guardarPresupuesto} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-6 border-b border-gray-100">
              ...
              <div>
                ...
                <input 
                  type="number" step="0.1" required value={coeficiente}
            ...
            <div>
              <div className="flex justify-between items-center mb-4">
                ...
                <button 
                  type="button" onClick={agregarFila}
                ...
                {items.map((item, index) => (
                  <div key={index} className="flex gap-3 items-end bg-gray-50 p-3 rounded-lg border border-gray-200">
                    <div className="flex-1">
                      ...
                      <select 
                        required value={item.materialId}
                    ...
                    <button 
                      type="button" onClick={() => eliminarFila(index)}
            ...
            <tbody>
              {historial.map((orden) => (
                <tr key={orden.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="p-4 font-medium text-gray-800">{orden.cliente.nombre}</td>
                  <td className="p-4 text-right font-bold text-blue-600">${Number(orden.costo_total).toLocaleString('es-AR')}</td>
                  <td className="p-4 text-center">
                    <button 
                      onClick={() => abrirImpresion(orden)}
                      className="inline-flex items-center gap-1 text-gray-600 hover:text-blue-600 bg-gray-100 hover:bg-blue-50 py-1.5 px-3 rounded-md text-xs font-medium transition-colors"
                    >
      ...
      {/* ========================================================================= */}
      {/* HOJA DE IMPRESIÓN (Invisible normalmente, visible en modal y al imprimir) */}
      {/* ========================================================================= */}
            ...
            <div className="flex justify-between items-center p-4 border-b border-gray-200 bg-gray-50 print:hidden">
              ...
              <div className="flex items-center gap-3">
                <button 
                  ...
                  className="bg-blue-600 hover:bg-blue-700 text-white font-medium py-1.5 px-4 rounded-md text-sm transition-colors flex items-center gap-1.5"
                >
            ...
            <div className="flex-1 overflow-y-auto p-8 print:overflow-visible print:p-0">
              <div className="border border-gray-200 p-8 rounded-lg max-w-[800px] mx-auto print:border-0 print:p-0">
                ...
                <div className="flex justify-between items-start border-b-2 border-gray-300 pb-6 mb-6">
                  <div>
                    {/* Buscamos el logo. Si no existe, muestra el fallback de texto */}
                    <img 
                      src="/logo.png" 
                      alt="Logo Taller" 
                      onError={(e) => { e.currentTarget.style.display = 'none'; }} // Se oculta si no existe
                ...
                <div className="bg-gray-50 p-4 rounded-lg border border-gray-100 mb-6 grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-bold text-gray-800 text-base">{presupuestoSeleccionado.cliente.nombre}</p>
                  </div>
                  <div className="text-right">
                    ...
                    {presupuestoSeleccionado.cliente.direccion && (
                      <div className="mt-1">
                        <span className="text-xs text-gray-400 uppercase font-semibold">Dirección</span>
                        <p className="text-gray-700">{presupuestoSeleccionado.cliente.direccion}</p>
                      </div>
                    )}
                  </div>
                </div>
                  ...
                  <table className="w-full border-collapse border border-gray-200">
                    ...
                    <tbody className="text-sm text-gray-700">
                      {presupuestoSeleccionado.items.map((item: any, idx: number) => {
                        const sub = Number(item.precio_congelado) * item.cantidad;
                        return (
                          <tr key={idx} className="border-b border-gray-100">
                            <td className="p-2.5 border border-gray-200 font-medium">{item.material.descripcion}</td>
                            <td className="p-2.5 border border-gray-200 text-center">{item.cantidad}</td>
                            <td className="p-2.5 border border-gray-200 text-center">{item.material.unidad_medida}</td>
                            <td className="p-2.5 border border-gray-200 text-right">${Number(item.precio_congelado).toLocaleString('es-AR')}</td>
                            <td className="p-2.5 border border-gray-200 text-right font-medium">${sub.toLocaleString('es-AR')}</td>
                          </tr>
                        );
                      })}
                    </tbody>
                ...
                <div className="flex justify-end mt-4">
                  <div className="w-80 text-sm space-y-2.5 border-t-2 border-gray-200 pt-4">
                    <div className="flex justify-between text-gray-600">
                      ...
                      <span>${Number(presupuestoSeleccionado.costo_materiales).toLocaleString('es-AR')}</span>
                    </div>
                    <div className="flex justify-between text-gray-600">
                      <span>Mano de Obra y Gastos (Coef. {presupuestoSeleccionado.coeficiente_mano_obra}x):</span>
                      <span>${(Number(presupuestoSeleccionado.costo_total) - Number(presupuestoSeleccionado.costo_materiales)).toLocaleString('es-AR')}</span>
                    </div>
                    <div className="flex justify-between font-extrabold text-lg text-blue-600 border-t border-gray-200 pt-2 bg-blue-50 px-3 py-1.5 rounded-md">
                      ...
                      <span>${Number(presupuestoSeleccionado.costo_total).toLocaleString('es-AR')}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-12 pt-8 border-t border-dashed border-gray-200 text-xs text-gray-500 space-y-4">
                  ...
                  <div className="flex justify-between pt-12">
                    ...
                    <div className="text-center w-48 border-t border-gray-300 pt-1 font-semibold text-gray-700">
                      Taller Responsable
                    </div>
                  </div>
                </div>
