...
export default function Materiales() {
  ...
  // LÓGICA DE BÚSQUEDA: Filtramos la lista original basándonos en lo que el usuario escribe
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
        ...
        <form onSubmit={guardarMaterial} className="flex flex-wrap gap-4 items-end">
          <div className="flex-1 min-w-[200px]">
            ...
            <input type="text" required value={descripcion} onChange={(e) => setDescripcion(e.target.value)} className="w-full border border-gray-300 rounded-md p-2 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Ej. Caño Estructural 40x40" />
          </div>
          <div className="w-32">
            <label className="block text-sm text-gray-600 mb-1">Unidad</label>
            <select value={unidadMedida} onChange={(e) => setUnidadMedida(e.target.value)} className="w-full border border-gray-300 rounded-md p-2 outline-none">
          ...
          <div className="w-40">
            ...
            <input type="number" required value={precioBase} onChange={(e) => setPrecioBase(e.target.value)} className="w-full border border-gray-300 rounded-md p-2 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="0.00" />
          </div>
          ...
          <button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-6 rounded-md transition-colors">Guardar</button>
        </form>
      </div>

      {/* Tarjeta de la Tabla CON BUSCADOR */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {/* Iteramos sobre el arreglo FILTRADO, no sobre el original */}
            ...
            {materialesFiltrados.length === 0 && (
              <tr>
                <td colSpan={4} className="p-8 text-center text-gray-500">
                  {busqueda ? 'No se encontraron materiales que coincidan con la búsqueda.' : 'No hay materiales registrados.'}
                </td>
              </tr>
            )}
          </tbody>
